...
## Purpose
SOUL is one logical cognitive organism composed of seven independent AI nuclei (N01..N07). Fusion means shared identity, discovery, routing, authorization, observability and cooperative execution. It does not mean collapsing the nuclei into one implementation.
...
The fused system exposes one logical capability fabric while preserving seven execution domains. A request may enter through N01, be delegated to any owning nucleus, invoke that nucleus's native runtime/tooling, and return a correlated result through the Mesh.

## Channels
Every nucleus has six logical IN channels and six logical OUT channels for the other six nuclei. Logical channel identity is independent of transport.

...
import { Activity, Brain, Network, Play, RefreshCw, Zap } from 'lucide-react';
    ...
    <ScrollArea className="h-full">
      <div className="p-3 space-y-3">
        ...
        <Card className="border-primary/20 bg-card/50">
          <CardHeader className="p-3 pb-2"><CardTitle className="text-[10px] font-mono">UNIVERSAL CAPABILITY EXECUTOR</CardTitle></CardHeader>
          <CardContent className="p-3 pt-0 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select value={target} onChange={e => { setTarget(e.target.value as Nucleus); setCapability(''); }} className="h-8 rounded border border-border bg-background px-2 text-[9px] font-mono">{nuclei.map(n => <option key={n}>{n}</option>)}</select>
              <select value={capability} onChange={e => setCapability(e.target.value)} className="h-8 rounded border border-border bg-background px-2 text-[9px] font-mono"><option value="">Select capability</option>{selected?.capabilities.map(c => <option key={c}>{c}</option>)}</select>
            </div>
            <Textarea value={payload} onChange={e => setPayload(e.target.value)} className="min-h-16 text-[9px] font-mono" placeholder="JSON payload" />
            <Button size="sm" className="h-7 text-[9px] gap-1" onClick={() => void executeCapability()} disabled={!capability || selected?.state !== 'READY'}><Play className="h-3 w-3" /> EXECUTE THROUGH MESH</Button>
            {execution && <pre className="max-h-36 overflow-auto rounded bg-black/30 p-2 text-[8px] font-mono whitespace-pre-wrap">{execution}</pre>}
          </CardContent>
        </Card>
        ...
        {nuclei.map(nucleus => {
          ...
          return <Card key={nucleus} className="border-border/30 bg-card/50"><CardContent className="p-2.5">
            <div className="flex items-center gap-2 text-[10px] font-mono"><span className={cn('h-2 w-2 rounded-full', positive ? 'bg-emerald-400' : state === 'OFFLINE' ? 'bg-red-400' : 'bg-amber-400')} /><strong>{nucleus}</strong><Badge variant="outline" className="ml-auto text-[8px]">{state}</Badge>{runtime?.latencyMs != null && <span className="text-muted-foreground">{runtime.latencyMs}ms</span>}</div>
            <div className="mt-2 text-[9px] text-muted-foreground break-all">{runtime?.endpoint ?? (nucleus === 'N01' ? 'APK / local hybrid gateway' : 'endpoint not configured')}</div>
            {runtime?.capabilities.length ? <div className="mt-2 flex flex-wrap gap-1">{runtime.capabilities.map(cap => <Badge key={cap} variant="secondary" className="text-[8px]">{cap}</Badge>)}</div> : null}
          </CardContent></Card>;
        })}
        <div className="text-[8px] text-muted-foreground font-mono leading-relaxed">READY requires a live capability-discovery response with matching correlation ID and EXECUTED proof. Source-code presence alone never becomes connectivity.</div>
      </div>
    </ScrollArea>
  );
}

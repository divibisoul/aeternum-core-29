 ...
 * 
 * NÃO é um modal. É um painel permanente embutido no layout principal.
 * Todos os 13 motores estão sempre visíveis e atualizando em tempo real.
 * Sem botões de toggle/expand - sempre ativo.
 */
...
export function AGIActivePanel() {
  ...
  // Continuous foreground update loop - NEVER pauses
  useEffect(() => {
      ...
      <div className="p-3 space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap text-[10px] font-mono">
          <Badge className="bg-emerald-500/20 text-emerald-400 text-[9px]">
            {metrics?.overall?.activeSubsystems ?? 0}/{metrics?.overall?.subsystems ?? 13} ONLINE
          </Badge>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-5 h-7">
            <TabsTrigger value="engines" className="text-[9px] px-1">Motores</TabsTrigger>
            ...
            <div className="grid grid-cols-2 gap-2">
              {metrics?.darwin && (
                <Card className="border-border/30 bg-card/50">
                  <CardContent className="p-2">
                    <div className="text-[10px] font-medium flex items-center gap-1 mb-1">
                    ...
                    <div className="grid grid-cols-2 gap-0.5 text-[9px]">
                      ...
                      <div><span className="text-muted-foreground">Avg</span><div className="font-mono text-emerald-400">{(metrics.darwin.avgFitness * 100).toFixed(0)}%</div></div>
                    </div>
                  </CardContent>
                </Card>
              )}
              {metrics?.safety && (
                <Card className="border-border/30 bg-card/50">
                  <CardContent className="p-2">
                    <div className="text-[10px] font-medium flex items-center gap-1 mb-1">
                      <Shield className="w-3 h-3 text-sky-400" /> Safety
                    </div>
                    <div className="text-[9px]">
                      <span className="font-mono text-emerald-400">{(metrics.safety.overallHealth * 100).toFixed(0)}%</span>
                    </div>
                    <Progress value={metrics.safety.overallHealth * 100} className="h-1 mt-1" />
                  </CardContent>
                </Card>
              )}
            </div>
          ...
          {/* === NEURAL TAB - Projeto Clareira + Consciência === */}
              ...
              <CardContent className="p-2 space-y-1">
                ...
                {neuralMetrics && (
                  <div className="space-y-0.5 text-[9px]">
                    <div className="flex justify-between"><span className="text-muted-foreground">Nós Ativos</span><span className="font-mono">{neuralMetrics.activeNodes}/{neuralMetrics.totalNodes}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Temperatura</span><span className="font-mono">{neuralMetrics.averageTemperature.toFixed(2)}</span></div>
                    ...
                    <div className="flex justify-between"><span className="text-muted-foreground">Carga</span><span className="font-mono">{(neuralMetrics.averageLoad * 100).toFixed(0)}%</span></div>
              ...
              <CardContent className="p-2 space-y-1">
                ...
                {conscienciaMetrics && (
                  <div className="space-y-1 text-[9px]">
                    <div className="flex justify-between"><span className="text-muted-foreground">Técnica (NNs)</span><span className="font-mono text-blue-400">{conscienciaMetrics.camadas.tecnico.reforco.iterations} iter</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Simbólica (Q)</span><span className="font-mono text-purple-400">{conscienciaMetrics.camadas.simbolico.totalUpdates} upd</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Filosófica (Ε)</span><span className="font-mono text-orange-400">{(conscienciaMetrics.camadas.filosofico.cobertura * 100).toFixed(0)}%</span></div>
                    ...
                    <div className="text-center text-muted-foreground">{conscienciaMetrics.processamentosTotal} processamentos</div>
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent className="p-2 space-y-1 text-[9px]">
                ...
                {metrics?.saiic && (
                  <>
                    ...
                    <div className="flex justify-between"><span className="text-muted-foreground">Isolados</span><span className="font-mono">{metrics.saiic.isolatedModules}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Loops</span><span className="font-mono text-red-400">{metrics.saiic.loopsDetected}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Latência Scan</span><span className="font-mono">{metrics.saiic.avgScanLatencyMs.toFixed(2)}ms</span></div>
                  </>
                )}
              </CardContent>
            </Card>
          ...
          <TabsContent value="mesh" className="mt-2 space-y-2">
            {metrics?.connectivity && (
              <Card className="border-border/30 bg-card/50">
                <CardContent className="p-2 space-y-0.5 text-[9px]">
                  <div className="text-[10px] font-medium flex items-center gap-1">
                    <Network className="w-3 h-3 text-cyan-400" /> Full-Mesh Connectivity
                  </div>
            ...
            {metrics?.resources && (
              <Card className="border-border/30 bg-card/50">
                <CardContent className="p-2 space-y-0.5 text-[9px]">
                  <div className="text-[10px] font-medium flex items-center gap-1">
                    <Cpu className="w-3 h-3 text-primary" /> Resource Manager
                  </div>
                  ...
                  {metrics.resources.hotModules?.length > 0 && (
                    <div className="flex justify-between"><span className="text-muted-foreground">Hot</span><span className="font-mono text-amber-400 truncate">{metrics.resources.hotModules.join(', ')}</span></div>
                  )}
                </CardContent>
              </Card>
            )}

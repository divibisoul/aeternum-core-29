...
function parseIntent(text: string, model: string): N01Intent {
  let parsed: unknown;
  try {
  ...
  const capabilities = Array.isArray(value.capabilities) ? value.capabilities.filter((item): item is string => typeof item === 'string' && item.trim()).map((item) => item.trim()).slice(0, 32) : [];
  ...
  return { intent, summary, capabilities, confidence, rawText: text, provider: 'google-gemini', model };
}

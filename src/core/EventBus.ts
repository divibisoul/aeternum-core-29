 ...
 *
 * AETERNUM custom events are carried through the explicit bridge envelope
 * below; this keeps one runtime event authority instead of creating a second bus.
 */
...
class EventBusImpl {
  ...
  on<K extends keyof AeternumEvents>(event: K, callback: EventCallback<AeternumEvents[K]>): UnsubscribeFn {
    return this.addListener(event, callback, false);
  }

  once<K extends keyof AeternumEvents>(event: K, callback: EventCallback<AeternumEvents[K]>): UnsubscribeFn {
  ...
  async emit<K extends keyof AeternumEvents>(event: K, data: AeternumEvents[K]): Promise<void> {
    ...
    await Promise.allSettled(copy.map(async sub => {
      try {
        await sub.callback(data);
      } catch (error) {
        console.error("[EventBus] " + String(event), error);
  ...
  private logEvent(event: string, data: unknown): void {
    ...
    if (import.meta.env.DEV) console.log("[EventBus] " + event, data);
  }
}

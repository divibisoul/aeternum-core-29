...
export type SoulMeshHubResult = { accepted: boolean; reason?: 'TARGET_NOT_REGISTERED' | 'CAPABILITY_NOT_ADVERTISED' };
...
export class SoulMeshHub {
  ...
  register(nucleus: SoulNucleus, handler: (message: SoulMeshMessage) => void | Promise<void>): () => void {
    ...
    return () => { if (this.handlers.get(nucleus) === handler) this.handlers.delete(nucleus); };
  }

  /** Routes locally registered nuclei through the same path as remote nuclei. */
  async send(message: SoulMeshMessage): Promise<SoulMeshHubResult> {
    if (message.kind === 'request' && message.capability) {
      ...
      if (advertised.length > 0 && !known) return { accepted: false, reason: 'CAPABILITY_NOT_ADVERTISED' };
      if (known && !known.request) return { accepted: false, reason: 'CAPABILITY_NOT_ADVERTISED' };
    }

    const localHandler = this.handlers.get(message.target);
    if (localHandler) { await localHandler(message); return { accepted: true }; }

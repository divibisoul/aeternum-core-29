...
/** HTTP transport for deployed nuclei. Inbound messages are delivered by the host endpoint. */
export class SoulMeshHttpTransport implements SoulMeshTransport {
  ...
  async send(message: SoulMeshMessage): Promise<void> {
    const response = await fetch(this.endpoint, { method: 'POST', headers: { 'content-type': 'application/json', ...this.headers }, body: JSON.stringify(message) });

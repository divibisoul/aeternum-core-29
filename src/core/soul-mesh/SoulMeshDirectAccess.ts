import { createSoulMeshMessage } from '../mesh/SoulMeshProtocol';
import { resolvePeerEndpoint } from '../mesh/SoulMeshPeerEndpoints';
...
export class SoulMeshDirectAccess {
  ...
  constructor(surface: SoulAccessSurface) { this.surface = surface; }
  channels(): SoulChannel[] { return SOUL_MESH_60_CHANNELS.map((channel) => ({ ...channel })); }
  ...
  async probeChannel(channel: SoulChannel): Promise<SoulChannelProbe> {
    ...
    const endpoint = resolvePeerEndpoint(peer, 'out');
    const message = createSoulMeshMessage({
      source: 'N01', target: peer, kind: 'request', capability: 'mesh.ping', channelId: channel.id,
      payload: { surface: this.surface }, correlationId,
    });
    ...
    try {
      ...
      const body = (await response.json()) as { correlationId?: string; proof?: string };
      const correlated = body.correlationId === correlationId;
      const reachable = response.ok && correlated;
      return { channel, surface: this.surface, configured, reachable, correlationId, proof: body.proof,

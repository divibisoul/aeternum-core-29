...
function circuitAllows(target:NucleusId){
 const openedAt=circuitOpenedAt.get(target);
 if(openedAt===undefined)return true;
 if(Date.now()-openedAt>=CIRCUIT_COOLDOWN_MS){circuitOpenedAt.delete(target);circuitFailures.delete(target);return true;}
 return false;
}
function recordSuccess(target:NucleusId){circuitFailures.delete(target);circuitOpenedAt.delete(target);}
function recordFailure(target:NucleusId){
 const failures=(circuitFailures.get(target)??0)+1;
 circuitFailures.set(target,failures);
 if(failures>=CIRCUIT_FAILURE_THRESHOLD)circuitOpenedAt.set(target,Date.now());
}
...
export async function sendTo(target:NucleusId,capability:string,payload:unknown,timeoutMs=15000):Promise<SoulMeshMessage>{
 ...
 for(let attempt=1;attempt<=MAX_ATTEMPTS;attempt++){
  const id=uuid();
  const message:SoulMeshMessage={protocol:'soul-mesh/1',contractVersion:'1.1.0',id,correlationId,source:'N01',target,kind:'request',capability,payload,timestamp:Date.now(),meta:{runtime:'aeternum-core-29',transport:'HTTP',encoding:'json',version:'1.1.0',traceId:correlationId}};
  const n=nonce();message.meta={...(message.meta??{}),nonce:n};
  const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
   const headers:Record<string,string>={'content-type':'application/json','x-soul-correlation-id':correlationId};
   if(secret){headers['x-soul-mesh-nonce']=n;headers['x-soul-mesh-hmac']=sign(message,n);}
   ...
   if(!response.ok||body.kind==='error'){
    ...
    if(!isRetryableStatus(response.status))throw new Error(detail);
   ...
   const retryable=error instanceof DOMException&&error.name==='AbortError'||messageText.startsWith('SOUL_MESH_REMOTE_ERROR:')&&/(^|:)(408|429|5\d\d)$/.test(messageText)||messageText.includes('fetch failed')||messageText.includes('network');
   if(!retryable||attempt===MAX_ATTEMPTS){recordFailure(target);throw error;}

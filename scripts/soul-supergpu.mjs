...
export function createSuperGPU({ resolveOwner, forward, self = 'N01' }) {
  ...
  async function execute(task, correlationId) {
    ...
    if (plan.owner === self) {
      return { ...plan, execution: 'local', output: { owner: self, capability: plan.capability, payload: task.payload }, startedAt, finishedAt: Date.now() };
    }
    const message = {
    ...
    return { ...plan, execution: 'remote', output, startedAt, finishedAt: Date.now() };
  }

...
async function uploadToWeb3Storage(input: HybridStorageInput): Promise<string> {
  ...
  const response = await fetch(`${WEB3_STORAGE_API_URL}/upload`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: form });
...
async function recordInSupabase(record: { cid: string; mimeType: string; nucleusId: string; metadata: Record<string, unknown>; gatewayUrl: string }): Promise<{ id: string; createdAt: string }> {
  ...
  const response = await fetch(`${supabaseUrl.replace(/\/$/, '')}/rest/v1/${STORAGE_TABLE}`, {
    ...
    body: JSON.stringify({ cid: record.cid, mime_type: record.mimeType, nucleus_id: record.nucleusId, metadata: record.metadata, gateway_url: record.gatewayUrl }),
  });
...
/** Canonical N01 artifact storage: IPFS content address + Supabase metadata, separate from vector-memory ownership. */

import { buildCapabilityLinks, type MeshCapabilityLink } from './SoulMeshCapabilityGraph';
import { SoulMeshPeerResilience } from './SoulMeshPeerResilience';
import type { TransportKind } from './HybridTransportRegistry';
...
export class SoulMeshOrchestrator {
  ...
  constructor(policy: Partial<OrchestrationPolicy> = {}, resilience = new SoulMeshPeerResilience()) {
    this.policy = { maxSteps: 16, ...policy };
  ...
  plan(
    ...
    const steps = requested.map((request, index) => {
      const link: MeshCapabilityLink | undefined = links.find(
        (candidate) => candidate.capability === request.capability && (!request.target || candidate.target === request.target),
      );
      if (!link) throw new Error(`CAPABILITY_ROUTE_UNAVAILABLE:${request.capability}`);
      if (this.policy.allowedTargets && !this.policy.allowedTargets.includes(link.target)) {
        throw new Error(`ORCHESTRATION_TARGET_NOT_ALLOWED:${link.target}`);
      }
      ...
      return {
        ...
        target: link.target,
        ...(request.dependsOn ? { dependsOn: request.dependsOn } : {}),
      } satisfies OrchestrationStep;
    });
    const ids = new Set(steps.map((step) => step.id));
    for (const step of steps) {
      for (const dependency of step.dependsOn ?? []) {
        if (!ids.has(dependency)) throw new Error(`ORCHESTRATION_UNKNOWN_DEPENDENCY:${dependency}`);
      }
    }
    return { steps };
  }

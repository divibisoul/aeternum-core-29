...
export interface BridgeRequest<T = unknown> { target: Exclude<SoulNodeId, 'N01' | 'BROADCAST'>; capability: string; payload: T; correlationId?: string; transport?: TransportKind; }
...
type NativeBridge = { dispatch?: (message: string) => void; complete?: (message: string) => void };
...
export function dispatchToNative<T>(request: BridgeRequest<T>, timeoutMs = 30000): Promise<BridgeCompletion> {
  ...
  return new Promise((resolve, reject) => {
    ...
    try { bridge.dispatch(JSON.stringify(envelope)); } catch (error) { clearTimeout(timer); pending.delete(envelope.correlationId); reject(error); }
  });
}
